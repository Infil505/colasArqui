...
var require_enterprise_maintenance_manager = __commonJS({
  "node_modules/@redis/client/dist/lib/client/enterprise-maintenance-manager.js"(exports2) {
    ...
    var promises_2 = require("timers/promises");
    var node_diagnostics_channel_1 = __importDefault(require("diagnostics_channel"));
    ...
    var EnterpriseMaintenanceManager = class {
      ...
      constructor(commandsQueue, client2, options) {
...
var require_socket = __commonJS({
  "node_modules/@redis/client/dist/lib/client/socket.js"(exports2) {
    ...
    var node_tls_1 = __importDefault(require("tls"));
    var errors_1 = require_errors();
...
var require_commands_queue = __commonJS({
  "node_modules/@redis/client/dist/lib/client/commands-queue.js"(exports2) {
    ...
    var RedisCommandsQueue = class _RedisCommandsQueue {
      ...
      addPushHandler(handler2) {
        this.#pushHandlers.push(handler2);
      }
...
var require_legacy_mode = __commonJS({
  "node_modules/@redis/client/dist/lib/client/legacy-mode.js"(exports2) {
    ...
    var RedisLegacyClient = class _RedisLegacyClient {
      ...
      constructor(client2) {
        this.#client = client2;
        const RESP = client2.options?.RESP ?? 2;
  ...
  "node_modules/@redis/client/dist/lib/client/cache.js"(exports2) {
    ...
    var BasicClientSideCache = class extends ClientSideCacheProvider {
      ...
      /* logic of how caching works:
        ...
        */
      async handleCache(client2, parser, fn, transformReply, typeMapping) {
        let reply;
        ...
        if (cacheEntry) {
          ...
          const promise = fn();
          cacheEntry = this.createPromiseEntry(client2, promise);
      ...
      createValueEntry(client2, value) {
        return new ClientSideCacheEntryValue(this.ttl, value);
      }
      createPromiseEntry(client2, sendCommandPromise) {
    ...
    var RedisClientPool = class _RedisClientPool extends node_events_1.EventEmitter {
      static #createCommand(command, resp) {
        ...
        return async function(...args) {
          const parser = new parser_1.BasicCommandParser();
          command.parseCommand(parser, ...args);
          return this.execute((client2) => client2._executeCommand(command, parser, this._commandOptions, transformReply));
        };
      }
      static #createModuleCommand(command, resp) {
        const transformReply = (0, commander_1.getTransformReply)(command, resp);
        return async function(...args) {
          const parser = new parser_1.BasicCommandParser();
          command.parseCommand(parser, ...args);
          return this._self.execute((client2) => client2._executeCommand(command, parser, this._self._commandOptions, transformReply));
        };
      }
      static #createFunctionCommand(name, fn, resp) {
        const prefix = (0, commander_1.functionArgumentsPrefix)(name, fn);
        ...
        return async function(...args) {
          ...
          parser.push(...prefix);
          fn.parseCommand(parser, ...args);
          return this._self.execute((client2) => client2._executeCommand(fn, parser, this._self._commandOptions, transformReply));
        };
      }
      static #createScriptCommand(script, resp) {
        ...
        return async function(...args) {
          ...
          script.parseCommand(parser, ...args);
          return this.execute((client2) => client2._executeScript(script, parser, this._commandOptions, transformReply));
        };
      }
      ...
      #cleanup() {
        ...
        for (let i = 0; i < toDestroy; i++) {
          const client2 = this.#idleClients.shift();
          client2.destroy();
        }
      }
      sendCommand(args, options) {
        return this.execute((client2) => client2.sendCommand(args, options));
      }
      MULTI() {
        return new this.Multi((commands, selectedDB) => this.execute((client2) => client2._executeMulti(commands, selectedDB)), (commands) => this.execute((client2) => client2._executePipeline(commands)), this._commandOptions?.typeMapping);
      }
      multi = this.MULTI;
      async close() {
        ...
        try {
          const promises = [];
          for (const client2 of this._self.#idleClients) {
            promises.push(client2.close());
          }
          for (const client2 of this._self.#clientsInUse) {
            promises.push(client2.close());
          }
          await Promise.all(promises);
  ...
  "node_modules/@redis/client/dist/lib/cluster/cluster-slots.js"(exports2) {
    ...
    var RedisClusterSlots = class {
      ...
      async #getShards(rootNode) {
        ...
        const client2 = await this.#clientFactory(options).on("error", (err) => this.#emit("error", err)).connect();
        try {
      ...
      #createClient(node, readonly = node.readonly) {
        ...
        const client2 = this.#clientFactory(this.#clientOptionsDefaults({
      ...
      #createNodeClient(node, readonly) {
        const client2 = node.client = this.#createClient(node, readonly);
        return node.connectPromise = client2.connect().finally(() => node.connectPromise = void 0);
      }
      nodeClient(node) {
       ...
       */
      quit() {
        return this.#destroy((client2) => client2.quit());
      }
      /**
       * @deprecated Use `destroy` instead.
       */
      disconnect() {
        return this.#destroy((client2) => client2.disconnect());
      }
      close() {
        return this.#destroy((client2) => client2.close());
      }
      destroy() {
        this.#isOpen = false;
      ...
      *#slotNodesIterator(slot) {
      ...
      async #initiatePubSubClient(toResubscribe) {
        const index = Math.floor(Math.random() * (this.masters.length + this.replicas.length)), node = index < this.masters.length ? this.masters[index] : this.replicas[index - this.masters.length], client2 = this.#createClient(node, false);
        this.pubSubNode = {
          address: node.address,
          client: client2,
          connectPromise: client2.connect().then(async (client3) => {
            if (toResubscribe) {
              await Promise.all([
                client3.extendPubSubListeners(pub_sub_1.PUBSUB_TYPE.CHANNELS, toResubscribe[pub_sub_1.PUBSUB_TYPE.CHANNELS]),
                client3.extendPubSubListeners(pub_sub_1.PUBSUB_TYPE.PATTERNS, toResubscribe[pub_sub_1.PUBSUB_TYPE.PATTERNS])
              ]);
            }
      ...
      async #initiateShardedPubSubClient(master) {
        const client2 = this.#createClient(master, false).on("server-sunsubscribe", async (channel, listeners) => {
          try {
            await this.rediscover(client2);
        ...
        master.pubSub = {
          client: client2,
          connectPromise: client2.connect().then((client3) => {
      ...
      async executeShardedUnsubscribeCommand(channel, unsubscribe) {
        ...
        if (!master.pubSub)
          return;
        const client2 = master.pubSub.connectPromise ? await master.pubSub.connectPromise : master.pubSub.client;
        await unsubscribe(client2);
        if (!client2.isPubSubActive) {
          client2.destroy();
          master.pubSub = void 0;
    ...
    var RedisCluster = class _RedisCluster extends node_events_1.EventEmitter {
      static #createCommand(command, resp) {
        ...
        return async function(...args) {
          const parser = new parser_1.BasicCommandParser();
          command.parseCommand(parser, ...args);
          return this._self._execute(parser.firstKey, command.IS_READ_ONLY, this._commandOptions, (client2, opts) => client2._executeCommand(command, parser, opts, transformReply));
        };
      }
      static #createModuleCommand(command, resp) {
        const transformReply = (0, commander_1.getTransformReply)(command, resp);
        return async function(...args) {
          const parser = new parser_1.BasicCommandParser();
          command.parseCommand(parser, ...args);
          return this._self._execute(parser.firstKey, command.IS_READ_ONLY, this._self._commandOptions, (client2, opts) => client2._executeCommand(command, parser, opts, transformReply));
        };
      }
      static #createFunctionCommand(name, fn, resp) {
        const prefix = (0, commander_1.functionArgumentsPrefix)(name, fn);
        ...
        return async function(...args) {
          ...
          parser.push(...prefix);
          fn.parseCommand(parser, ...args);
          return this._self._execute(parser.firstKey, fn.IS_READ_ONLY, this._self._commandOptions, (client2, opts) => client2._executeCommand(fn, parser, opts, transformReply));
        };
      }
      static #createScriptCommand(script, resp) {
        const prefix = (0, commander_1.scriptArgumentsPrefix)(script);
        ...
        return async function(...args) {
          ...
          parser.push(...prefix);
          script.parseCommand(parser, ...args);
          return this._self._execute(parser.firstKey, script.IS_READ_ONLY, this._commandOptions, (client2, opts) => client2._executeScript(script, parser, opts, transformReply));
        };
      }
      static #SingleEntryCache = new single_entry_cache_1.default();
      static factory(config) {
      ...
      // }
      _handleAsk(fn) {
        return async (client2, options) => {
          const chainId = Symbol("asking chain");
          const opts = options ? { ...options } : {};
          opts.chainId = chainId;
          const ret = await Promise.all([
            client2.sendCommand([ASKING_1.ASKING_CMD], { chainId }),
            fn(client2, opts)
          ]);
          return ret[1];
        };
      ...
      async _execute(firstKey, isReadonly, options, fn) {
        ...
        let client2 = await this._slots.getClient(firstKey, isReadonly);
        let i = 0;
        let myFn = fn;
        while (true) {
          try {
            return await myFn(client2, options);
          } catch (err) {
            myFn = fn;
            ...
            if (err.message.startsWith("ASK")) {
              ...
              if (!redirectTo) {
                await this._slots.rediscover(client2);
            ...
            if (err.message.startsWith("MOVED")) {
              await this._slots.rediscover(client2);
              client2 = await this._slots.getClient(firstKey, isReadonly);
              continue;
            }
            throw err;
          }
      ...
      async sendCommand(firstKey, isReadonly, args, options) {
        ...
        return this._self._execute(firstKey, isReadonly, opts, (client2, opts2) => client2.sendCommand(args, opts2));
      }
      MULTI(routing) {
        return new this.Multi(async (firstKey, isReadonly, commands) => {
          const client2 = await this._self._slots.getClient(firstKey, isReadonly);
          return client2._executeMulti(commands);
        }, async (firstKey, isReadonly, commands) => {
          const client2 = await this._self._slots.getClient(firstKey, isReadonly);
          return client2._executePipeline(commands);
      ...
      async UNSUBSCRIBE(channels, listener, bufferMode) {
        return this._self._slots.executeUnsubscribeCommand((client2) => client2.UNSUBSCRIBE(channels, listener, bufferMode));
      }
      unsubscribe = this.UNSUBSCRIBE;
      ...
      async PUNSUBSCRIBE(patterns, listener, bufferMode) {
        return this._self._slots.executeUnsubscribeCommand((client2) => client2.PUNSUBSCRIBE(patterns, listener, bufferMode));
      }
      pUnsubscribe = this.PUNSUBSCRIBE;
      async SSUBSCRIBE(channels, listener, bufferMode) {
        ...
        let client2 = await this._self._slots.getShardedPubSubClient(firstChannel);
        for (let i = 0; ; i++) {
          try {
            ...
            if (err.message.startsWith("MOVED")) {
              await this._self._slots.rediscover(client2);
              client2 = await this._self._slots.getShardedPubSubClient(firstChannel);
              continue;
            }
            throw err;
          }
      ...
      sSubscribe = this.SSUBSCRIBE;
      SUNSUBSCRIBE(channels, listener, bufferMode) {
        return this._self._slots.executeShardedUnsubscribeCommand(Array.isArray(channels) ? channels[0] : channels, (client2) => client2.SUNSUBSCRIBE(channels, listener, bufferMode));
      }
  ...
  "node_modules/@redis/client/dist/lib/sentinel/utils.js"(exports2) {
    ...
    function createCommand(command, resp) {
      ...
      return async function(...args) {
        const parser = new parser_1.BasicCommandParser();
        command.parseCommand(parser, ...args);
        return this._self._execute(command.IS_READ_ONLY, (client2) => client2._executeCommand(command, parser, this.commandOptions, transformReply));
      };
    }
    ...
    function createFunctionCommand(name, fn, resp) {
      ...
      return async function(...args) {
        ...
        parser.push(...prefix);
        fn.parseCommand(parser, ...args);
        return this._self._execute(fn.IS_READ_ONLY, (client2) => client2._executeCommand(fn, parser, this._self.commandOptions, transformReply));
      };
    }
    ...
    function createModuleCommand(command, resp) {
      ...
      return async function(...args) {
        const parser = new parser_1.BasicCommandParser();
        command.parseCommand(parser, ...args);
        return this._self._execute(command.IS_READ_ONLY, (client2) => client2._executeCommand(command, parser, this._self.commandOptions, transformReply));
      };
    }
    ...
    function createScriptCommand(script, resp) {
      ...
      return async function(...args) {
        ...
        parser.push(...prefix);
        script.parseCommand(parser, ...args);
        return this._self._execute(script.IS_READ_ONLY, (client2) => client2._executeScript(script, parser, this.commandOptions, transformReply));
      };
    }
...
var require_pub_sub_proxy = __commonJS({
  "node_modules/@redis/client/dist/lib/sentinel/pub-sub-proxy.js"(exports2) {
    ...
    var node_events_1 = __importDefault(require("events"));
    var pub_sub_1 = require_pub_sub();
    var client_1 = __importDefault(require_client());
    var PubSubProxy = class extends node_events_1.default {
      ...
      async #initiatePubSubClient(withSubscriptions = false) {
        const client2 = this.#createClient().on("error", this.#onError);
        const connectPromise = client2.connect().then(async (client3) => {
          if (this.#state?.client !== client3) {
            client3.destroy();
            return this.#state?.connectPromise;
          }
          if (withSubscriptions && this.#subscriptions) {
            await Promise.all([
              client3.extendPubSubListeners(pub_sub_1.PUBSUB_TYPE.CHANNELS, this.#subscriptions[pub_sub_1.PUBSUB_TYPE.CHANNELS]),
              client3.extendPubSubListeners(pub_sub_1.PUBSUB_TYPE.PATTERNS, this.#subscriptions[pub_sub_1.PUBSUB_TYPE.PATTERNS])
            ]);
          }
          if (this.#state.client !== client3) {
            client3.destroy();
      ...
      #executeCommand(fn) {
        const client2 = this.#getPubSubClient();
        if (client2 instanceof client_1.default) {
          return fn(client2);
        }
        return client2.then((client3) => {
          if (client3 === void 0)
            return;
          return fn(client3);
        }).catch((err) => {
      ...
      subscribe(channels, listener, bufferMode) {
        return this.#executeCommand((client2) => client2.SUBSCRIBE(channels, listener, bufferMode));
      }
      #unsubscribe(fn) {
        return this.#executeCommand(async (client2) => {
          const reply = await fn(client2);
          if (!client2.isPubSubActive) {
            client2.destroy();
            this.#state = void 0;
          }
      ...
      async unsubscribe(channels, listener, bufferMode) {
        return this.#unsubscribe((client2) => client2.UNSUBSCRIBE(channels, listener, bufferMode));
      }
      async pSubscribe(patterns, listener, bufferMode) {
        return this.#executeCommand((client2) => client2.PSUBSCRIBE(patterns, listener, bufferMode));
      }
      async pUnsubscribe(patterns, listener, bufferMode) {
        return this.#unsubscribe((client2) => client2.PUNSUBSCRIBE(patterns, listener, bufferMode));
      }
      destroy() {
  ...
  "node_modules/@redis/client/dist/lib/sentinel/index.js"(exports2) {
    ...
    var RedisSentinelClient = class _RedisSentinelClient {
      ...
      async sendCommand(isReadonly, args, options) {
        return this._execute(isReadonly, (client2) => client2.sendCommand(args, options));
      }
      /**
       * @internal
       */
      async _executePipeline(isReadonly, commands) {
        return this._execute(isReadonly, (client2) => client2._executePipeline(commands));
      }
      /**f
        * @internal
        */
      async _executeMulti(isReadonly, commands) {
        return this._execute(isReadonly, (client2) => client2._executeMulti(commands));
      }
      MULTI() {
      ...
      WATCH(key) {
        ...
        return this._execute(false, (client2) => client2.watch(key));
      }
      watch = this.WATCH;
      UNWATCH() {
        if (this._self.#clientInfo === void 0) {
        ...
        return this._execute(false, (client2) => client2.unwatch());
      }
      unwatch = this.UNWATCH;
      /**
    ...
    var RedisSentinelInternal = class extends node_events_1.EventEmitter {
      ...
      releaseClientLease(clientInfo) {
        const client2 = this.#masterClients[clientInfo.id];
        if (client2 !== void 0) {
          const dirtyPromise = client2.resetIfDirty();
          if (dirtyPromise) {
      ...
      async execute(fn, clientInfo) {
        ...
        while (true) {
          ...
          const client2 = this.#getClient(clientInfo);
          if (!client2.isReady) {
            await this.#reset();
            continue;
          }
          const sockOpts = client2.options?.socket;
          ...
          try {
            ...
            if (clientInfo !== void 0 && (err.message.startsWith("READONLY") || !client2.isReady)) {
              await this.#reset();
              continue;
            }
            throw err;
      ...
      async #createPubSub(client2) {
        await client2.pSubscribe(["switch-master", "[-+]sdown", "+slave", "+sentinel", "[-+]odown", "+slave-reconf-done"], (message, channel) => {
      ...
      async close() {
        ...
        for (const client2 of this.#masterClients) {
          if (client2.isOpen) {
            promises.push(client2.close());
          }
        }
        this.#masterClients = [];
        for (const client2 of this.#replicaClients) {
          if (client2.isOpen) {
            promises.push(client2.close());
          }
        }
      ...
      async observe() {
        for (const node of this.#sentinelRootNodes) {
          ...
          try {
            ...
            client2 = this.#createClient(node, this.#sentinelClientOptions, false);
            client2.on("error", (err) => this.emit("error", `obseve client error: ${err}`));
            await client2.connect();
            this.#trace(`observe: connected to sentinel`);
            const [sentinelData, masterData, replicaData] = await Promise.all([
              client2.sentinel.sentinelSentinels(this.#name),
              client2.sentinel.sentinelMaster(this.#name),
              client2.sentinel.sentinelReplicas(this.#name)
            ]);
            ...
            if (client2 !== void 0 && client2.isOpen) {
      ...
      async transform(analyzed) {
        ...
        if (analyzed.sentinelToOpen) {
          ...
          const client2 = this.#createClient(analyzed.sentinelToOpen, this.#sentinelClientOptions, false);
          client2.on("error", (err) => {
            ...
            const event2 = {
              type: "SENTINEL",
              node: (0, utils_1.clientSocketToNode)(client2.options.socket),
              error: err
            };
            this.emit("client-error", event2);
            this.#handleSentinelFailure(node);
          }).on("end", () => this.#handleSentinelFailure(node));
          this.#sentinelClient = client2;
          this.#trace(`transform: adding sentinel client connect() to promise list`);
          const promise = this.#sentinelClient.connect().then((client3) => {
            return this.#createPubSub(client3);
          });
        ...
        if (analyzed.masterToOpen) {
          ...
          for (const client2 of this.#masterClients) {
            masterWatches.push(client2.isWatching || client2.isDirtyWatch);
            if (client2.isOpen) {
              client2.destroy();
            }
          }
          ...
          for (let i = 0; i < this.#masterPoolSize; i++) {
            ...
            const client2 = this.#createClient(analyzed.masterToOpen, this.#nodeClientOptions);
            client2.on("error", (err) => {
              if (this.#passthroughClientErrorEvents) {
            ...
            if (masterWatches[i]) {
              client2.setDirtyWatch("sentinel config changed in middle of a WATCH Transaction");
            }
            this.#masterClients.push(client2);
            masterPromises.push(client2.connect());
        ...
        if (analyzed.replicasToOpen.size != 0) {
          for (const [node, size] of analyzed.replicasToOpen) {
            for (let i = 0; i < size; i++) {
              const client2 = this.#createClient(node, this.#nodeClientOptions);
              client2.on("error", (err) => {
                if (this.#passthroughClientErrorEvents) {
              ...
              this.#replicaClients.push(client2);
              promises.push(client2.connect());
    ...
    var RedisSentinelFactory = class extends node_events_1.EventEmitter {
      ...
      async updateSentinelRootNodes() {
        for (const node of this.#sentinelRootNodes) {
          const client2 = client_1.default.create({
          ...
          try {
            const sentinelData = await client2.sentinel.sentinelSentinels(this.options.name);
      ...
      async getMasterNode() {
        ...
        for (const node of this.#sentinelRootNodes) {
          ...
          connected = true;
          try {
            const masterData = await client2.sentinel.sentinelMaster(this.options.name);
      ...
      async getReplicaNodes() {
        ...
        for (const node of this.#sentinelRootNodes) {
          ...
          connected = true;
          try {
            const replicaData = await client2.sentinel.sentinelReplicas(this.options.name);
...
// netlify/functions/publishers-get.ts
var publishers_get_exports = {};
__export(publishers_get_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(publishers_get_exports);

// netlify/functions/lib/db.ts
var import_redis = __toESM(require_dist2());
var client = null;
  ...
  client = (0, import_redis.createClient)({
    ...
    socket: {
      ...
      reconnectStrategy: (retries) => {
        if (retries > 10) {
          return new Error("Demasiados intentos de reconexi\xF3n");
        }
        return retries * 100;
      }
    }
...
//# sourceMappingURL=publishers-get.js.map
